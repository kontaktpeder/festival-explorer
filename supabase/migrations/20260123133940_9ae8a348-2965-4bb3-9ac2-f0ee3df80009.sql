-- Fix: delete_user_safely must handle access_invitations.invited_by FK before deleting profile
...
BEGIN
  ...
  -- 9. Invitations: revoke pending + reassign inviter to a platform admin (prevents FK violation)
  WITH updated AS (
  ...
  IF v_new_owner IS NOT NULL THEN
    UPDATE public.access_invitations
    SET invited_by = v_new_owner
    WHERE invited_by = v_profile_id;
  ELSE
    -- last resort: delete invitations sent by this user (required because invited_by is NOT NULL)
