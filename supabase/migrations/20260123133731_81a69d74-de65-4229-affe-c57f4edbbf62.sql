-- Fix delete_user_safely to handle staff_roles before profile deletion
...
BEGIN
  ...
  -- 1. Transfer entities
  FOR v_entity IN 
    SELECT e.id, e.created_by FROM public.entities e WHERE e.created_by = v_profile_id AND e.is_system = false
  LOOP
  ...
  -- 9. Revoke invitations
  WITH updated AS (
    UPDATE public.access_invitations SET status = 'revoked' WHERE invited_by = v_profile_id AND status = 'pending' RETURNING 1
  )
