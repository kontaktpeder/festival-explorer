...
BEGIN
  ...
  -- Step: festival finance (correct order: unlink → attachments → entries → books)
  ...
  -- Step: project_members / venue_members
  ...
  -- Step: Clear rider asset refs on program slots (disable trigger with correct name)
  ALTER TABLE public.event_program_slots DISABLE TRIGGER trg_enforce_runsheet_live_fields;
  ...
  ALTER TABLE public.event_program_slots ENABLE TRIGGER trg_enforce_runsheet_live_fields;

  -- Step: asset_handles
  DELETE FROM public.asset_handles WHERE created_by = v_profile_id;
  GET DIAGNOSTICS v_deleted_asset_handles = ROW_COUNT;

  -- Step: Transfer entities to team members or fallback
  ...
  -- Step: access_invitations (nullify invited_user_id, handle invited_by)
