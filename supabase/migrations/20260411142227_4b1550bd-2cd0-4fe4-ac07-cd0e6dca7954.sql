...
BEGIN
  ...
  -- 0b. Clear rider asset references on entities owned by this user
  ...
  -- 0c. Clear rider asset references on program slots
  -- Must disable the runsheet live-field trigger which blocks plan-field changes
  -- for non-admin/non-editor callers (SECURITY DEFINER context has no auth.uid())
  ALTER TABLE public.event_program_slots DISABLE TRIGGER enforce_runsheet_live_fields;
  ...
  ALTER TABLE public.event_program_slots ENABLE TRIGGER enforce_runsheet_live_fields;

  -- 0d. Delete asset_handles created by this user
  WITH deleted AS (
    DELETE FROM public.asset_handles WHERE created_by = v_profile_id RETURNING 1
  )
  SELECT COUNT(*) INTO v_deleted_asset_handles FROM deleted;
