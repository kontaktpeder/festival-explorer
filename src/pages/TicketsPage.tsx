...
export default function TicketsPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: types,
        error
      } = await supabase.from("ticket_types").select("id, code, name, description, price_nok, capacity, visible").eq("visible", true).order("sort_order");
      ...
      const {
        data: tickets,
        error: ticketsError
      } = await supabase.from("tickets").select("ticket_type_id").neq("status", "CANCELLED");
  ...
  const renderTicketCard = (type: TicketTypeWithCount) => {
    ...
    return <div key={type.id} className={`ticket-card rounded-lg ${soldOut ? "opacity-50 cursor-not-allowed" : "cursor-pointer"} ${selectedType === type.id ? "selected" : ""}`} onClick={() => !soldOut && setSelectedType(type.id)}>
        <div className="flex items-center justify-between p-4">
          <div className="space-y-0.5">
            <p className="text-base font-semibold text-foreground">{type.name}</p>
            <p className="text-xs text-muted-foreground">{type.description}</p>
            {soldOut ? <p className="text-xs font-semibold text-destructive">Utsolgt</p> : showRemaining ? <p className="text-xs font-semibold text-accent">{remaining} igjen</p> : null}
          </div>
  ...
  return <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {/* Festival passes */}
        {festivalPasses.length > 0 && <div className="space-y-3">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Festivalpass // BOILER ROOM </h2>
        ...
        {selectedType && <div className="ticket-card rounded-lg">
