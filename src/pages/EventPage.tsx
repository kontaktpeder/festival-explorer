  ...
  return (
    <PageLayout>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 pt-5 md:pt-8 relative z-10">
        ...
        <p className="text-muted-foreground/60 text-base mt-4 max-w-lg">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3 mt-5 text-base text-foreground/70">
          ...
          {event.venue && (
            <Link to={`/venue/${event.venue.slug}`} className="flex items-center gap-2 hover:text-accent transition-colors">
              <MapPin className="w-4 h-4 text-muted-foreground/40" strokeWidth={1.5} />
              {event.venue.name}
            </Link>
          )}
          {(() => {
            ...
            return (
              <span className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-muted-foreground/40" strokeWidth={1.5} />
                {ageLimit}
              </span>
            );
          })()}
          {(event as any).cloakroom_available === true && (
            <span className="flex items-center gap-2">
              <Shirt className="w-4 h-4 text-muted-foreground/40" strokeWidth={1.5} />
              Garderobe
            </span>
          )}
        </div>
              ...
              {(event as any).programSlots && (event as any).programSlots.length > 0 ? (
                ...
                <>
                  {event.lineup && event.lineup.length > 0 && (
                    <div>
                      <h2 className="text-xs md:text-sm font-semibold uppercase tracking-widest text-muted-foreground/50 mb-2">
                        Program
                      </h2>
                      <p className="text-muted-foreground/60 text-base mb-6">
