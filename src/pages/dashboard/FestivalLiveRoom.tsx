...
export default function FestivalLiveRoom() {
  ...
  const { data: canOperate = false } = useQuery({
    ...
    enabled: !!id,
    queryFn: async () => {
      const { data } = await supabase.rpc("can_operate_runsheet_festival" as any, {
        p_festival_id: id!,
      });
      return !!data;
    },
  ...
  const handleAction = useCallback(
    async (slotId: string, action: LiveAction) => {
      ...
      try {
        ...
        await queryClient.invalidateQueries({ queryKey: ["festival-live-slots", id] });
      } catch (e: any) {
