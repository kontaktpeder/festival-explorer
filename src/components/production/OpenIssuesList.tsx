...
export interface IssueSlotContext {
...
function issueTitle(issue: EventIssueRow, ctx?: IssueSlotContext): string {
  const label = ISSUE_LABELS[issue.type] ?? issue.type.replace(/_/g, " ");
  const parts = [label];
  if (ctx?.performerName) parts.push(`– ${ctx.performerName}`);
  if (ctx?.stageLabel) parts.push(`(${ctx.stageLabel})`);
  return parts.join(" ");
}
...
export function OpenIssuesList(props: {
  ...
  issueContextBySlotId?: Record<string, IssueSlotContext>;
}) {
  ...
  return (
    <div className="space-y-2">
      ...
      {props.issues.map((issue) => {
        const ctx = props.issueContextBySlotId?.[issue.related_program_slot_id];
        return (
          <div
            key={issue.id}
