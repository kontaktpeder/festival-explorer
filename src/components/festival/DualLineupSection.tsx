...
export function DualLineupSection({
  ...
  const {
    ...
    return {
      festivalArtists: festival,
      boilerRoomArtists: boilerRoom
    };
  }, [artists]);
  return <>
      {/* ============================================ */}
      ...
      <div className="relative">
        ...
        <div className="relative">
          {festivalArtists.map((artist, index) => <ArtistPosterBlock key={artist.id} artist={artist} index={index} variant="festival" />)}
        </div>
      ...
      <div className="relative bg-black">
        ...
        {/* Artist poster blocks */}
        <div className="relative">
          {boilerRoomArtists.map((artist, index) => <ArtistPosterBlock key={artist.id} artist={artist} index={index} variant="boilerroom" />)}
          
          {/* If no boiler room artists, show placeholder */}
          {boilerRoomArtists.length === 0 && <div className="min-h-[40vh] flex items-center justify-center">
      ...
      {festivalTeam && (festivalTeam.hostRoles.length > 0 || festivalTeam.backstage.length > 0) && <section className="relative py-24 md:py-32 px-6 bg-black overflow-hidden">
          {/* Subtle accent glow */}
